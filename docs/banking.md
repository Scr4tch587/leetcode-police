...
- **2nd+ problem on the same game day** (**today only**; day rolls at **4:00 AM**): `bankedProblems += 1` on ingest,
  or on the next sync/check if both solves are already saved in Firestore.
- At **4:00 AM** for **yesterday’s game day** with **N** submissions:
...
- Game-day cutoff: `functions/src/lib/game.ts` → `resolveUserDay` (`dailyProcessor.ts` at 4:05 AM)
